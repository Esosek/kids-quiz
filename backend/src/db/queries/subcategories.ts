...
export async function getFreeSubcategories(): Promise<Subcategory[]> {
  try {
    return await db.select().from(subcategories).where(eq(subcategories.unlockPrice, 0))
  } catch (error) {
    throw new Error('Retrieving free subcategories failed')
  }
}

export async function createSubcategory(label: string, imageURL: string, categoryId?: string, unlockPrice?: number): Promise<Subcategory> {
  try {
    if (categoryId) {
    ...
    const [createdSubcategory] = await db.insert(subcategories).values({ label, categoryId, unlockPrice, imageURL }).onConflictDoNothing().returning()
    ...
    console.log(error)
    throw new Error('Creating subcategory failed')
  }
}
